...
from src.utils import (disable_proxy, download_item, get_extension_from_url,
                       get_time_now_as_day, get_time_now_as_hour, unquote_sid)
from src.validators import ListObjectType


def parse_args(*args):
    parser = ArgumentParser("InstagramDownloader")
    parser.add_argument("users", metavar="categories", type=str, nargs="*", help="The categories that has the sessionid and the users to scrape", default=["TEST"])
    parser.add_argument("--output", "-o", dest="download_path", type=str, help="The folder to store downloaded files in.", default=MEDIA_PATH)
    parser.add_argument("--download-limit", "-l", dest="download_limit", type=int, help="The number of calls to do per one call. Keep it at a low number to avoid huge payload, but not too low to avoid rate limiting. (Default 3)", default=LIMIT)

    input_group = parser.add_argument_group(title="Input Methods")
    input_group.add_argument("--input-file", "-f", type=str, help="The json file to load session and users from", default=os.path.join("data", "list.json"))
    input_group.add_argument("--session-id", "-e", type=str, help="Your session id to login as.", default="")
    input_group.add_argument("--users", "-u", metavar="USERNAME", dest="usernames", type=str, nargs="+", help="The usernames to download", default=[])

    save_group = parser.add_argument_group(title="Download Switches")
    save_group.add_argument("--disable-save-story", "--no-story", "-s", dest="dl_story", action="store_false", help="Disable downloading Stories if on")
    save_group.add_argument("--disable-save-posts", "--no-posts", "-p", dest="dl_posts", action="store_false", help="Disable downloading Posts if on")
    save_group.add_argument("--disable-save-highlights", "--no-highlights", "-i", dest="dl_high", action="store_false", help="Disable downloading Highlights if on")

    save_only_group = save_group.add_mutually_exclusive_group()
    save_only_group.add_argument("--save-story-only", "--only-story", "-S", dest="story_only", action="store_true", help="Download stories only")
    save_only_group.add_argument("--save-posts-only", "--only-posts", "-P", dest="posts_only", action="store_true", help="Download posts only")
    save_only_group.add_argument("--save-highlights-only", "--only-highlights", "-I", dest="high_only", action="store_true", help="Download Highlights only")
...
if __name__ == "__main__":
    ...
    if passed_session_id or passed_users:
        if not passed_session_id or not passed_users:
            raise ValueError("When Input File is not specified, both session id and users must be passed.")
        usernames_list: Dict[str, ListObjectType] = {
            "passed": {
                "sessionid": passed_session_id,
                "users": passed_users,
            }
        }
        args.user = session_users = ["passed"]
    else:
        try:
    ...
    for session_user in session_users:
        ...
        for username in usernames:
            ...
            if username in username_mappings.values():
                ...
                pro_pic_path = os.path.join(downloads_folder, username, "profile_pics", time_str)
            ...
            pro_pic_path = os.path.join(pro_pic_path, get_time_now_as_day() + "." + get_extension_from_url(profile_pic))
        ...
        for i in range(0, len(usernames) if dl_story else 0, download_limit):
            ...
            for story_data, user_id in instagram.parse_reels_data(data, username_mappings):
                ...
                instagram.download_list(story_data, username_mappings, "stories", downloads_folder)
        ...
        for user_id, username in username_mappings.items() if dl_posts else []:
            ...
            instagram.download_list(full_posts, username_mappings, posts_folder, downloads_folder)
        ...
        for user_id, username in username_mappings.items() if dl_high else []:
            ...
            for i in range(0, len(highlights_ids), download_limit): # Walk 3 at a time
                print(f"Getting page {i//download_limit +1} / {ceil(len(highlights_ids)/download_limit)}")
                ...
                for j, highlight in enumerate(data["reels"].values()):
                    ...
                    highlights_folder_full_path = os.path.join(downloads_folder, username, highlights_folder)
                    print(f"Getting highlight {h_id} ({j+1}/{download_limit})")
                    instagram.download_list(highlights_data[h_id]["reels"], username_mappings, highlights_folder, downloads_folder)
